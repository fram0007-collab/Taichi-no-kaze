...
- The `/safe-zones` and `/pois` endpoints perform geographical checks using `haversine_m`.
- Any safe zone POI falling inside an active threat circle is excluded from the returned payload.
