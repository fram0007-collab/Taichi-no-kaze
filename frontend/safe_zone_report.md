...
- Computes `threatZoneCircles` dynamically using React `useMemo` from `predictions` and `allZones` (filtering by active layer and risk scores).
- Filters `safeZones` list using `calculateDistanceKm` to suppress safe zones that are geographically inside any active threat zone circle.
