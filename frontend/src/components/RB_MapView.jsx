...
export default function MapView({ 
  ...
  const [activeLayers, setActiveLayers] = useState({
    ...
    waterways: false,  // Waterways is disabled by default
    earthquakes: false // Earthquakes is disabled by default
  });
  ...
  return (
    <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-premium border border-slate-800">
      ...
      >
        ...
        {/* Render ALL zones — green=low, yellow=medium, red=high */}
        {/* allZones drives the map display; predictions drive sidebar/alerts */}
        {allZones.map(zs => {
          const zone = zs.zone;
          if (!zone) return null;
          ...
          // Find matching alert for click-through
          const matchedPred = predictions.find(p => p.zone?.zone_id === zone.zone_id || p.zone?.id === zone.zone_id);

          // Map display_severity to the existing risk style keys
          const severityMap = { HIGH: 'High', MEDIUM: 'Medium', LOW: 'Low' };
          const riskKey = severityMap[zs.display_severity] || 'Low';
              ...
              <Tooltip sticky>
                <div className="font-sans p-1 text-slate-100">
                  ...
                  <div className="text-[11px] text-slate-300 mt-1">
                    Threat: <span className="font-semibold text-slate-100">{zs.dominant_risk || 'Normal'}</span>
                  </div>
                  <div className="text-[11px] text-slate-300">
                    Confidence: <span className="font-semibold text-slate-100">{zs.overall_risk_score?.toFixed(1)}%</span>
                  </div>
                  {distanceStr && (
